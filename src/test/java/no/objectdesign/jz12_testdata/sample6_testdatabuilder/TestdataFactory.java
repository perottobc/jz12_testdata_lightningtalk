...
public class TestdataFactory {
	...
	public Integer insert(CustomerTestdata customerTestdata) {
		int maxId = jdbcTemplate.queryForInt( customerTestdata.sqlSelectMaxId() );
		
		int customerId = maxId+1;
		jdbcTemplate.execute(customerTestdata.asSqlInsert(customerId));
		
		for (ShoppingListTestdata shoppingListTestdata : customerTestdata
			.getShoppingList()) {
	...
	public void insert(ShoppingListTestdata shoppingListTestdata) {
		int maxId = jdbcTemplate.queryForInt( shoppingListTestdata.sqlSelectMaxId() );
		
		int shoppingListId = maxId+1;
		jdbcTemplate.execute(shoppingListTestdata.asSqlInsert(shoppingListId));
		
		for (ItemTestdata itemTestdata : shoppingListTestdata.getItems()) {
	...
	public void insert(ItemTestdata itemTestdata) {
		int maxId = jdbcTemplate.queryForInt( itemTestdata.sqlSelectMaxId() );
